const iResp = require('../../utils/response.interface.js');

const carbonTransactionService = require('../../services/carbonTrading/carbonTransaction.js');

const getList = async (req, res) => {
    try {
        const result = await carbonTransactionService.getList();

        if (!result.success) {
            res.status(result.code).send(result)
        }

        res.status(result.code).send(result)
    } catch (error) {
        res.status(500).send(iResp.buildErrorResponse(500, 'Something wrong', error));
    }
}

const getById = async (req, res) => {
    try {
        console.log(req.params.salesProposalId)
        const result = await carbonTransactionService.getById(req.params.salesProposalId);

        if (!result.success) {
            res.status(result.code).send(result)
        }

        res.status(result.code).send(result)
    } catch (error) {
        res.status(500).send(iResp.buildErrorResponse(500, 'Something wrong', error));
    }
}

const create = async (req, res) => {
    try {
        console.log(req.body)
        const result = await carbonTransactionService.create(req.body);

        if (!result.success) {
            res.status(result.code).send(result)
        }

        res.status(result.code).send(result)
    } catch (error) {
        res.status(500).send(iResp.buildErrorResponse(500, 'Something wrong', error));
    }
}

const update = async (req, res) => {
    try {
        console.log(req.params.salesProposalId, req.body)
        const result = await carbonTransactionService.update(req.params.salesProposalId, req.body);

        if (!result.success) {
            res.status(result.code).send(result)
        }

        res.status(result.code).send(result)
    } catch (error) {
        res.status(500).send(iResp.buildErrorResponse(500, 'Something wrong', error));
    }
}

const remove = async (req, res) => {
    try {
        const result = await carbonTransactionService.remove(req.params.salesProposalId);
        console.log(req.params.salesProposalId)

        if (!result.success) {
            res.status(result.code).send(result)
        }

        res.status(result.code).send(result)
    } catch (error) {
        res.status(500).send(iResp.buildErrorResponse(500, 'Something wrong', error));
    }
}

module.exports = { getList, getById, create, update, remove }