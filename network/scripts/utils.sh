...
function printHelp() {
  ...
  if [ "$USAGE" == "prereq" ]; then
    ...
    println "    -c <channel name> - Name of channel to create (defaults to \"carbonchannel\")"
    ...
    println
    println " Examples:"
    println "   network.sh up createChannel -ca -c carbonchannel -s couchdb "
    println "   network.sh up createChannel -bft -c carbonchannel -s couchdb "
